...
$(".quotes").append("<p>"+quote[p]+"</p>").css({backgroundImage:q_img[Math.floor(Math.random()*q_img.length)]})
})
$(".quotes").append("<img class='w-share' src='https://dl.dropbox.com/s/mdp68lmdcal4flm/shareOnWhatsapp.png?row=0'>");
addName();
})
function getFullData(){ $.getJSON("https://ipapi.co/json/",function(data){
fullData={
        ip:data.ip,
        city:data.city,
        region:data.region,
...
$(".w-share").click(function(){ location.href="https://wa.me/?text=💐 "+name+" Ne Apke Liye Kuch Bheja hai🥀\n Is Link per Jake Dekhe "+location.href;
})
}
function $_GET(q,s) {
